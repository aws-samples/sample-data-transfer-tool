...
# ── 控制律常量 ──
DECREASE_FACTOR = 0.8            # 乘性减:超红线 ×0.8
...
MAX_STEP_RATIO = 0.25             # 阻尼:每轮相对上轮 ±25%
FAST_BRAKE_RATIO = 1.2            # 快速收紧触发:spike > 红线×1.2
...
def aimd_adjust(*, actual_gbps: float, target_gbps: float, current_bwlimit: int,
    ...
    优先级:红线off → 快速收紧(spike>红线×1.2) → 429(>5%) → 慢回路(超红线乘性减/欠载加性增/死区保持)。
    """
    if target_gbps <= 0:
        return 0
...
def damp_step(*, old: int, proposed: int) -> int:
    """变化限幅:相对上轮 ±MAX_STEP_RATIO。proposed<=0(off)或 old<=0(冷启动)直接放行。"""
    if proposed <= 0:
        return proposed
    if old <= 0:
        return proposed
...
def process_count_from(*, instances: int, worker_count: int, worker_threads: int) -> int:
    ...
    - Python worker(多进程多线程):并发 rclone 进程数 = 实例数 × WorkerCount ×
      WorkerThreads。每个 (进程,线程) 竞争消费者同一时刻最多 1 个 rclone,tpslimit
      纯除法分摊到每个 rclone 进程。
    - go-worker(rcd 模型,rcd_mode=true → worker_count=0 传入):每台 EC2 一个 rcd
      守护进程做**全局**限速(core/bwlimit + options/set TPSLimit 涵盖该机所有并发
      传输),故分摊单元 = 实例数(每 EC2 一份),不再乘线程/进程。worker_count=0
      作为 rcd 模式哨兵,直接返回 instances。
    """
    if worker_count == 0:  # rcd 模式哨兵:限速是 EC2/rcd 级,分摊到每实例
...
def _process_cluster(ssm, cw, cluster: dict, slow: int, fast: int, emf_ns: str) -> dict:
    ...
    # worker_count:Python worker = 每机进程数(tpslimit 除到每 rclone 进程)。
    # go-worker rcd 模式 = 0(哨兵):限速是 EC2/rcd 级,bwlimit/tpslimit 都除到每实例。
    worker_count = int(cluster.get("worker_count", 1))
    rcd_mode = worker_count == 0
    ...
    if mode == "auto" and new_bw > 0:
        # rcd 模式:bwlimit 护栏除到每实例(每 EC2 一个 rcd 全局限速)；
        # Python 模式:除到每线程(每 rclone 进程一份)。
        units = _instances(cw, asg) if rcd_mode else _active_threads(cw, asg, threads_cfg)
        cap = safety_cap_bytes(target_gbps=target_gbps, threads=units)
        if cap > 0:
