...
- **四态观测走 ops 日志**：EMF 指标路径已删除（省 CloudWatch 费用）；SUCCESS/RETRYABLE/FATAL/UNKNOWN
  与 record_fail/delete_fail 计数由 worker 周期打进 worker-ops 日志组，用 `errors` 命令或 Logs Insights 查。
